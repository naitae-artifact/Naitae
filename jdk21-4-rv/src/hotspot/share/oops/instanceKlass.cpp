...
static inline bool is_indexing_tree_class(const Symbol* class_name,
                                           const ClassLoaderData* loader_data) {
  return (class_name == vmSymbols::java_lang_rv_IndexingTree() &&
          loader_data->is_the_null_class_loader_data());
}
...
InstanceKlass* InstanceKlass::allocate_instance_klass(const ClassFileParser& parser, TRAPS) {
  ...
  } else if (is_indexing_tree_class(class_name, loader_data)) {
    // RV-Monitor indexing tree - java.lang.rv.IndexingTree
    ik = new (loader_data, size, THREAD) InstanceIndexingTreeKlass(parser);
