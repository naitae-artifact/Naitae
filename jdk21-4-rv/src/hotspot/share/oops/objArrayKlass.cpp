...
ObjArrayKlass* ObjArrayKlass::allocate_objArray_klass(ClassLoaderData* loader_data,
  ...
  // Initialize instance variables. Route to WeakObjArrayKlass if the
  // element type is tagged as a weak-array carrier.
  ObjArrayKlass* oak;
  if (weak) {
    oak = WeakObjArrayKlass::allocate(loader_data, n, element_klass, name, CHECK_NULL);
  } else {
    oak = ObjArrayKlass::allocate(loader_data, n, element_klass, name, CHECK_NULL);
  }
...
ObjArrayKlass::ObjArrayKlass(int n, Klass* element_klass, Symbol* name)
