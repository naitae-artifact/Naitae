...
class ObjArrayKlass : public ArrayKlass {
 ...
 protected:
  // Subclass-callable constructor that lets a derived klass record its
  // own KlassKind. Used by WeakObjArrayKlass.
 ...
 public:
  ...
  // Allocation. If `weak` is true, the returned klass is a WeakObjArrayKlass
  // whose slot scanning is suppressed during phases declared by an active
  // WeakArrayScanScope (commit that adds WeakObjArrayKlass).
  static ObjArrayKlass* allocate_objArray_klass(ClassLoaderData* loader_data,
                                                int n, Klass* element_klass,
                                                bool weak, TRAPS);
  static ObjArrayKlass* allocate_objArray_klass(ClassLoaderData* loader_data,
                                                int n, Klass* element_klass, TRAPS) {
    return allocate_objArray_klass(loader_data, n, element_klass, false, THREAD);
  }
