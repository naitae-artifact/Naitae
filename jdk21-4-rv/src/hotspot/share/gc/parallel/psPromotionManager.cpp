...
void PSPromotionManager::process_array_chunk(PartialArrayScanTask task) {
  ...
  // Defense-in-depth against stolen or already-pushed continuation tasks
  // for IndexingTree bucket arrays. The primary gate lives in
  // copy_to_survivor_space, which steers these arrays into push_contents;
  // this catches any chunk task that slipped through.
  if (WeakArrayScanScope::in(WeakArrayScanScope::YoungScan) &&
      old->klass()->is_weak_objArray_klass()) {
    return;
  }
