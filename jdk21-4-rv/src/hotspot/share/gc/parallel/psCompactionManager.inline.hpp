...
inline void ParCompactionManager::follow_array(objArrayOop obj, int index) {
  // Suppress strong tracing of IndexingTree bucket arrays during full-GC
  // marking — matches the suppression in ObjArrayKlass::oop_oop_iterate.
  // PSParallelCompact's array marking iterates slots manually (see
  // follow_array_specialized) and bypasses klass dispatch, so we need to
  // gate it here too.  Cleanup runs in ReferenceProcessor and uses
  // is_alive (mark bits) to decide what survives.
  if (obj->klass()->is_weak_objArray_klass() &&
      WeakArrayScanScope::in(WeakArrayScanScope::StrongMark)) {
    return;
  }
