...
inline oop PSPromotionManager::copy_unmarked_to_survivor_space(oop o,
  ...
  if (forwardee == nullptr) {  // forwardee is null when forwarding is successful
    ...
    //
    // IndexingTree bucket arrays must not be chunked during young GC:
    // process_array_chunk_work iterates slots via raw base()+offset and
    // bypasses ObjArrayKlass::oop_oop_iterate, defeating the weak-tracing
    // gate in WeakObjArrayKlass::oop_oop_iterate. Force push_contents, which
    // goes through klass dispatch and skips the bucket elements correctly.
    // Symmetric to ParCompactionManager::follow_array's full-GC gate.
    bool is_idx_table = WeakArrayScanScope::in(WeakArrayScanScope::YoungScan) &&
        new_obj->klass()->is_weak_objArray_klass();
    if (!is_idx_table &&
        new_obj_size > _min_array_size_for_chunking &&
        new_obj->is_objArray() &&
        PSChunkLargeArrays) {
      // we'll chunk it
