...
// RefProcTask that runs IndexingTree cleanup using the collector's own
// is_alive/keep_alive closures.  Workers race on a shared atomic counter
// to claim one tree at a time (granularity = single tree), so cleanup
// parallelizes whenever the registry has more than one tree.
//
// MT correctness depends on every worker calling complete_gc->do_void()
// exactly once at the end.  The collector's complete_gc is wired to a
// shared TaskTerminator that needs every worker to join the steal-loop
// for forward progress; if idle workers (those that claimed no trees)
// returned early, the busy worker would deadlock waiting for steal
// partners.  We therefore pass nullptr for complete_gc into the
// per-tree work and run a single converging drain on the way out.
class IndexingTreeCleanupTask : public RefProcTask {
  volatile uint _claim_counter;
public:
  IndexingTreeCleanupTask(ReferenceProcessor& ref_processor)
    : RefProcTask(ref_processor, nullptr),
      _claim_counter(0) {}
  ...
  void rp_work(uint worker_id,
    ...
    IndexingTreeManager::clean_trees_par_indexed(&_claim_counter, is_alive, keep_alive,
                                                 /*complete_gc=*/ nullptr);
    // Converge: every worker drives the steal-loop until the terminator
    // agrees nothing is left.  Idle workers help drain the busy worker's
    // per-worker mark stack via work-stealing.
    complete_gc->do_void();
  }
};
...
ReferenceProcessorStats ReferenceProcessor::process_discovered_references(RefProcProxyTask& proxy_task,
  ...
  // IndexingTree cleanup: remove entries whose keys are unreachable
  // outside the tree chain, then keep_alive surviving chains.
  //
  // Path 2 (card scanning of the bucket array) is suppressed by
  // WeakObjArrayKlass::oop_oop_iterate during young GC and full-GC
  // strong marking.  At this point:
  //   - From-space entries are intact (the chain was not self-traced)
  //   - is_alive(key) tells us whether the key was reached through a
  //     non-tree path (root, other dirty card, etc.)
  //
  // The collector's own is_alive/keep_alive/complete_gc closures are
  // passed through the proxy_task.  Each tree is owned by exactly one
  // worker via OopStorage::ParState's claim counter, so per-tree state
  // (size, table slots, entry next ptrs) needs no extra synchronization.
  if (WeakArrayScanScope::in(WeakArrayScanScope::StrongMark) ||
    ...
    if (processing_is_mt()) {
      proxy_task.prepare_run_task(idx_task, num_queues(), RefProcThreadModel::Multi, true /* marks_oops_alive */);
      WorkerThreads* workers = Universe::heap()->safepoint_workers();
      assert(workers != nullptr, "MT refproc requires safepoint workers");
      workers->run_task(&proxy_task, num_queues());
    } else {
      proxy_task.prepare_run_task(idx_task, 1, RefProcThreadModel::Single, true /* marks_oops_alive */);
      proxy_task.work(0);
    }
  }
