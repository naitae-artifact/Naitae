...
// support for indexing tree

int java_lang_rv_IndexingTree::_size_offset;
int java_lang_rv_IndexingTree::_id_offset;
int java_lang_rv_IndexingTree::_upThreshold_offset;
int java_lang_rv_IndexingTree::_downThreshold_offset;
int java_lang_rv_IndexingTree::_table_offset;
int java_lang_rv_IndexingTree::_capacity_offset;

#define INDEXINGTREE_FIELDS_DO(macro) \
  macro(_size_offset,          k, "size",          int_signature,    false); \
  macro(_id_offset,            k, "id",            int_signature,    false); \
  macro(_upThreshold_offset,   k, "upThreshold",   int_signature,    false); \
  macro(_downThreshold_offset, k, "downThreshold", int_signature,    false); \
  macro(_table_offset,         k, "table",         indexingtreeentry_array_signature, false); \
  macro(_capacity_offset,      k, "capacity",      int_signature,    false);

void java_lang_rv_IndexingTree::compute_offsets() {
  InstanceKlass* k = vmClasses::IndexingTree_klass();
  INDEXINGTREE_FIELDS_DO(FIELD_COMPUTE_OFFSET);
}

#if INCLUDE_CDS
void java_lang_rv_IndexingTree::serialize_offsets(SerializeClosure* f) {
  INDEXINGTREE_FIELDS_DO(FIELD_SERIALIZE_OFFSET);
}
#endif

// support for indexing tree entry

int java_lang_rv_IndexingTreeEntry::_hashCode_offset;
int java_lang_rv_IndexingTreeEntry::_keys_offset;
int java_lang_rv_IndexingTreeEntry::_value_offset;
int java_lang_rv_IndexingTreeEntry::_next_offset;

#define INDEXINGTREEENTRY_FIELDS_DO(macro) \
  macro(_hashCode_offset, k, "hashCode", int_signature,              false); \
  macro(_keys_offset,     k, "keys",     object_array_signature,     false); \
  macro(_value_offset,    k, "value",    object_signature,           false); \
  macro(_next_offset,     k, "next",     indexingtreeentry_signature, false);

void java_lang_rv_IndexingTreeEntry::compute_offsets() {
  InstanceKlass* k = vmClasses::IndexingTreeEntry_klass();
  INDEXINGTREEENTRY_FIELDS_DO(FIELD_COMPUTE_OFFSET);
}

#if INCLUDE_CDS
void java_lang_rv_IndexingTreeEntry::serialize_offsets(SerializeClosure* f) {
  INDEXINGTREEENTRY_FIELDS_DO(FIELD_SERIALIZE_OFFSET);
}
#endif
...
#define BASIC_JAVA_CLASSES_DO_PART2(f) \
  f(java_lang_rv_IndexingTree) \
  f(java_lang_rv_IndexingTreeEntry) \
  f(java_lang_System) \
...
void javaClasses_init() {
  ...
  InstanceIndexingTreeKlass::update_nonstatic_oop_maps(vmClasses::IndexingTree_klass());
}
