...
// java.lang.rv.IndexingTree / java.lang.rv.WeakObjArray //////////

// Cached WeakObjArrayKlass instances, lazily created.  Held outside the
// element type's _array_klasses cache so that the regular ObjArrayKlass
// for the same element type (used by ordinary `new T[N]` paths) remains
// undisturbed.  Lookup is single-writer via MultiArray_lock to match the
// pattern in InstanceKlass::array_klass.
static WeakObjArrayKlass* _indexing_tree_entry_weak_array_klass = nullptr;
static WeakObjArrayKlass* _object_weak_array_klass             = nullptr;

static WeakObjArrayKlass* get_or_create_weak_array_klass(Symbol* element_class_name,
                                                          WeakObjArrayKlass** cache,
                                                          TRAPS) {
  if (Atomic::load_acquire(cache) != nullptr) return *cache;

  Klass* element_klass = SystemDictionary::resolve_or_fail(
      element_class_name, true, CHECK_NULL);
  InstanceKlass* ik = InstanceKlass::cast(element_klass);

  MutexLocker ml(THREAD, MultiArray_lock);
  if (*cache != nullptr) return *cache;

  ObjArrayKlass* oak = ObjArrayKlass::allocate_objArray_klass(
      ik->class_loader_data(), 1, ik, /*weak=*/true, CHECK_NULL);
  assert(oak->is_weak_objArray_klass(), "must be WeakObjArrayKlass");
  Atomic::release_store(cache, (WeakObjArrayKlass*) oak);
  return *cache;
}

static objArrayOop alloc_indexing_table(jint capacity, TRAPS) {
  WeakObjArrayKlass* k = get_or_create_weak_array_klass(
      vmSymbols::java_lang_rv_IndexingTreeEntry(),
      &_indexing_tree_entry_weak_array_klass, CHECK_NULL);
  // Cast to base; WeakObjArrayKlass declares a static `allocate` factory
  // that hides the inherited instance allocate(int, TRAPS) via C++ name
  // lookup, so we go through the ObjArrayKlass view.
  return ObjArrayKlass::cast(k)->allocate(capacity, CHECK_NULL);
}

JVM_ENTRY(jobjectArray, JVM_WeakObjArrayAllocate(JNIEnv* env, jclass cls, jint length))
  WeakObjArrayKlass* k = get_or_create_weak_array_klass(
      vmSymbols::java_lang_Object(),
      &_object_weak_array_klass, CHECK_NULL);
  objArrayOop arr = ObjArrayKlass::cast(k)->allocate(length, CHECK_NULL);
  return (jobjectArray) JNIHandles::make_local(THREAD, arr);
JVM_END

JVM_ENTRY(void, JVM_IndexingTreeInit(JNIEnv* env, jobject tree, jint capacity))
  Handle tree_h(THREAD, JNIHandles::resolve_non_null(tree));
  objArrayOop arr = alloc_indexing_table(capacity, CHECK);
  Handle arr_h(THREAD, arr);
  // Register before publishing the table. add_tree may safepoint; arr_h keeps
  // the unpublished weak table live, and cleanup sees table == null until the
  // tree is fully published.
  IndexingTreeManager::add_tree(tree_h());
  java_lang_rv_IndexingTree::set_table(tree_h(), objArrayOop(arr_h()));
JVM_END

JVM_ENTRY(jobjectArray, JVM_IndexingTreeAllocTable(JNIEnv* env, jclass cls, jint capacity))
  objArrayOop arr = alloc_indexing_table(capacity, CHECK_NULL);
  return (jobjectArray) JNIHandles::make_local(THREAD, arr);
JVM_END

// Atomic resize transaction.  Allocation may safepoint, so we keep oops in
// handles across it; then in a no-safepoint section move every chain from
// old to new and publish capacity/thresholds/table together.  Closes the
// window where mutators in Java adjustCapacity could expose entries reachable
// only through an unpublished WeakObjArrayKlass-typed table whose elements
// GC scan-suppresses (oop_oop_iterate early-returns under YoungScan).
JVM_ENTRY(void, JVM_IndexingTreeAdjustCapacity(JNIEnv* env, jobject this_jobj,
...

  {
    ...
    // Match Java's hashIndex exactly.  alloc_indexing_table allocates an
    // array of exactly `capacity` elements (no padding), so table.length
    // equals the logical capacity and capacity is always a power of two
    // (starts at DEFAULT_CAPACITY, only doubled/halved).  Masking with
    // (new_capacity - 1) is therefore exact and matches the Java side, which
    // masks the loaded table's length.
